<template>
  ...
  <el-col :span="9">
    <div v-if="data.type === 0">
      <el-tag>  {{data.knowledgeName}}</el-tag>
    </div>
    <div  v-if="data.type === 1">
      <el-tag type="success">  {{data.topic}}</el-tag>
    </div>
        </el-col>
      </el-row>
    </el-header>
    <el-main>
      ...
      <el-row>
        <el-tag>发布时间：{{data.createTime | filterTime}}</el-tag>
      ...
      <el-row :gutter="20" v-show="isShow">
        <el-col :span="16">
          <div class="edit_container" align="center">
            <quill-editor v-model="commentText"
                          ref="myQuillEditor"
                          class="editer"
                          :options="editorOption" @ready="onEditorReady($event) " style="height: 100px;float: right;margin: 50px">
            </quill-editor>
          </div>
        </el-col>
        <el-col :span="2">
          <el-button type="success" @click="saveComment" style="margin: 50px"> 确定</el-button>
        </el-col>
      </el-row>
    </el-main>
    <el-footer>
      ...
      <el-row :gutter="20" v-show="isReply" style="background-color:aliceblue">
        <el-col :span="16">
          <div class="edit_container">
            <quill-editor v-model="replyText"
                          ref="myQuillEditor"
                          class="editer"
                          :options="editorOption" @ready="onEditorReady($event) " style="height: 70px;margin: 70px">
            </quill-editor>
          </div>
        </el-col>
        <el-col :span="1">
          <el-button type="success" @click="replyComment" style="margin: 70px"> 确定</el-button>
        </el-col>
      </el-row>
    </el-footer>
  </el-container>
