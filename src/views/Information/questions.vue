<template>
  <el-container>
    ...
    <el-main >
      <div style="float: left" v-for="direction in subjectList" :key="direction">
        <h3>{{direction.label}}</h3>
        <div v-for="subject in direction.children" :key="subject" style="float: left">
          <el-button style="margin: 3px 3px" :class="(subjests.indexOf(subject.value) === -1)? '' :'active'"
                     @click="select(subject.value)"> {{subject.label}}
          </el-button>
        </div>
      </div>
    </el-main>
    <el-footer>
        <el-row :gutter="24">
          ...
          <el-col :span="3" > <el-rate v-model="form.degree" :colors="colors"></el-rate></el-col>
          ...
          <el-col :span="3" ><el-button   @click="intoPractice()">开始练习</el-button> </el-col>
        </el-row>
    </el-footer>
  </el-container>
</template>
  ...
  export default {
    ...
    methods: {
      intoPractice () {
        ...
        this.$api.createTestBySubject(this.form).then(res => {
